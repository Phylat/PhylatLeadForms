<template>
  <div>
    <component v-for="(field, index) in schema" :key="index" :is="field.fieldType" :value="formData[field.name]" @input="updateForm(field.name, $event)" @selectedInput="selectedUpdateForm(field.name, $event)" v-bind="field">
    </component>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      selectedUpdateForm(fieldName, value) {
        console.log(event.target.value);
        console.log(value);
        // console.log(fieldName.selectedIndex);
        this.$set(this.formData, fieldName, value);
        this.$emit("input", this.formData);
      },
      nextStep(i) {
